...
import OfficerFormModal from "./AddOfficerForm";
...
const OfficerTable = ({ data }) => {
    ...
    const showingEnd = Math.min(currentPage * limit, isTotalOfficer);

    // Page size options
    const pageSizeOptions = [5, 10, 20, 50];

    useEffect(() => {
        FetchOfficerData(currentPage, limit, searchTerm, dateFrom, dateTo);
    ...
    const handleFormSubmit = async (e, submittedData) => {
        ...
        if (isEditing) {
            result = await UpdateOfficer(submittedData._id, submittedData);
        } else {
            result = await AddOfficer(submittedData);
        }
    ...
    const handleConfirmDelete = async () => {
        if (isDeleteID) {
            setIsLoading(true);
            const result = await DeleteOfficer(isDeleteID);
            setIsLoading(false);
                    ...
                    <div className="flex flex-col items-center justify-between gap-4 border-t border-slate-200 p-4 dark:border-slate-700 sm:flex-row">
                        <div className="flex flex-wrap items-center gap-4">
                            <div className="text-sm text-slate-600 dark:text-slate-400">
                                ...
                                of <span className="font-medium">{isTotalOfficer}</span>
                            </div>
                        </div>
