...
import AddAdminFormModal from "./AddAdminFormModal";
import LoadingOverlay from "../../../../ReusableFolder/LoadingOverlay";
import SuccessFailed from "../../../../ReusableFolder/SuccessandField";
import StatusVerification from "../../../../ReusableFolder/StatusModal";
import { Database } from "lucide-react";

const AdminTable = ({ data, bgtheme, FontColor }) => {
    const {
        AddAdmin,
        DeleteAdmin,
        customError,
    ...
    const showingEnd = Math.min(currentPage * limit, isTotalAdmin);

    useEffect(() => {
        FetchAdminData(currentPage, limit, searchTerm, dateFrom, dateTo);
    ...
    const handleFormSubmit = async (e, submittedData) => {
        ...
        if (isEditing) {
            result = await UpdateAdmin(submittedData._id, submittedData);
        } else {
            result = await AddAdmin(submittedData);
        }
    ...
    const handleConfirmDelete = async () => {
        if (isDeleteID) {
            setIsLoading(true);
            const result = await DeleteAdmin(isDeleteID);
            setIsLoading(false);
                ...
                <div className="card-body p-0">
                    ...
                    <div className="flex flex-col items-center justify-between gap-2 border-t border-slate-200 p-2 text-[10px] dark:border-slate-700 sm:flex-row sm:gap-3 sm:p-3 sm:text-xs">
                        <div className="text-slate-600 dark:text-slate-400">
                            ...
                            of <span className="font-medium">{isTotalAdmin}</span>
                        </div>
