...
const ParticipantTable = () => {
  ...
  const formatFullName = (admin) =>
    `${admin.first_name} ${admin.middle_name || ""} ${admin.last_name} ${admin.extention || ""}`.trim();
    ...
    <>
      ...
      <div className="card border border-pink-300/50 bg-gradient-to-br from-pink-50/30 to-blue-50/30 dark:border-pink-700/50 dark:from-pink-900/20 dark:to-blue-900/20">
        <div className="card-header flex flex-col gap-4 p-4 sm:flex-row sm:items-center">
          ...
          <div className="flex w-full flex-wrap gap-2 sm:w-auto items-center">
            <div className="relative">
              ...
              />
              <label className="absolute -top-2 left-3 bg-white px-1 text-xs text-slate-500 dark:bg-slate-800 dark:text-slate-400">From</label>
            </div>
            ...
            <div className="relative">
              ...
              />
              <label className="absolute -top-2 left-3 bg-white px-1 text-xs text-slate-500 dark:bg-slate-800 dark:text-slate-400">To</label>
            </div>
              ...
              <tbody className="table-body">
                {isParticipant && isParticipant.length > 0 ? (
                  isParticipant.map((admin, index) => (
                    <tr key={admin._id} className="table-row border-b border-pink-200/30 hover:bg-pink-100/20 dark:border-pink-900/30 dark:hover:bg-pink-900/10">
                  ...
                  <tr>
                    <td colSpan="7" className="border border-dashed border-pink-300 py-10 dark:border-pink-700/50">
          ...
          <div className="flex flex-col items-center justify-between gap-4 border-t border-slate-200 p-4 dark:border-slate-700 sm:flex-row">
            <div className="flex flex-wrap items-center gap-4">
              <div className="text-sm text-slate-600 dark:text-slate-400">
                Showing <span className="font-medium">{showingStart}-{showingEnd}</span> of <span className="font-medium">{isTotalParticipant}</span>
              </div>
            </div>
        ...
        <SuccessFailed isOpen={showModal} onClose={() => setShowModal(false)} status={modalStatus} errorMessage={customError} />
        <StatusVerification isOpen={isVerification} onConfirmDelete={handleConfirmDelete} onClose={() => setVerification(false)} />
      </div>
    </>
  );
};
