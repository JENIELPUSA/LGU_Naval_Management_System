...
const FBPageEmbed = () => {
  ...
  return (
    <motion.div
      ...
      className={`rounded-lg overflow-hidden max-w-[400px] mx-auto ${styles.container}`}
      role="complementary"
      aria-label={t('facebookPage')}
    >
      {/* HEADER */}
      <div 
        className={`p-5 flex items-center gap-3 ${styles.header}`}
        onFocus={handleHeaderFocus}
      >
        <div className={`p-2 rounded-lg ${
          accessibility.isHighContrast ? "bg-yellow-400 text-black" : "bg-white/20"
        }`}>
        ...
        <div>
          ...
          <p className={`text-sm ${
            accessibility.isHighContrast ? "text-gray-200" : "text-blue-100"
          }`}>
            {t('stayUpdated')}
          </p>
        </div>
      ...
      {/* EMBEDDED PAGE */}
      <div className={`p-3 flex justify-center ${styles.content}`}>
        <div className="relative">
          {/* Loading state */}
          {!iframeLoaded && (
            <div 
              className="w-[340px] min-h-[700px] flex items-center justify-center rounded-xl bg-gray-200"
              aria-live="polite"
              aria-label={t('loadingFacebook')}
            >
              <div className="text-center">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto mb-2"></div>
                <p className={accessibility.isHighContrast ? "text-gray-800" : "text-gray-600"}>
                  {t('loadingFacebook')}
                </p>
              </div>
            </div>
          ...
          <motion.iframe
            ...
            src={src}
            className={`w-[340px] min-h-[700px] rounded-xl ${
              iframeLoaded ? 'block' : 'absolute inset-0'
            }`}
      ...
      <div className={`p-4 border-t ${
        accessibility.isHighContrast ? "border-gray-600 bg-gray-100" : "border-gray-200"
      }`}>
        <a
          href={pageUrl}
          target="_blank"
          rel="noopener noreferrer"
          className={`flex items-center justify-center gap-2 px-4 py-2 rounded-lg font-semibold transition-colors ${styles.button} focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2`}
        ...
        <p className={`text-xs text-center mt-2 ${
          accessibility.isHighContrast ? "text-gray-700" : "text-gray-500"
        }`}>
          {t('latestUpdates')}
        </p>
      </div>
