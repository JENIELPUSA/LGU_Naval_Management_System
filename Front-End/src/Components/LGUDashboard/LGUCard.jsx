...
const LguCard = ({ icon, title, value, change, description, gradient, progress }) => {
    return (
...
const LguCardGrid = () => {
  ...
  // Ensure we have data
  const count = LguStatusCount[0] || { approved: 0, rejected: 0, pending: 0 };
  const total = count.approved + count.rejected + count.pending || 1;
  ...
  const statsData = [
    {
      ...
      progress: 100, // total events = 100%
    },
    {
      icon: <Users size={26} />,
      title: "Total Rejected Events",
      value: count.rejected,
      change: "+18%",
      description: "Total events rejected",
      gradient: "from-pink-500 via-purple-500 to-blue-500",
      progress: Math.round((count.rejected / total) * 100), // calculate %
    },
    {
      ...
      change: "+32%",
      description: "Total events approved",
      gradient: "from-blue-500 to-pink-500",
      progress: Math.round((count.approved / total) * 100),
    },
    {
      ...
      value: count.pending,
      change: "+8%",
      description: "Events pending review",
      gradient: "from-pink-500 to-blue-400",
      progress: Math.round((count.pending / total) * 100),
    },
  ];
